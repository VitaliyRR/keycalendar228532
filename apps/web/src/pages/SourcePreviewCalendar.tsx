...
export function SourcePreviewCalendar({orgId,start,end,days,search,explicitStart,onJump}:{orgId:string;start:string;end:string;days:string[];search:string;explicitStart:boolean;onJump:(day:string)=>void}) {
  ...
  return <>
    <div className="notice notice-warning source-preview-banner" role="status"><strong>Данные RealtyCalendar · только чтение.</strong><p>{data?`${data.coverage.propertyCount} объектов и ${data.coverage.reservationCount} броней. `:'Загружаем данные. '}Показаны даты бронирований из RealtyCalendar; изменение занятости здесь недоступно.</p></div>
    {resource.loading&&<Loading label="Загружаем шахматку RealtyCalendar…"/>}
    {Boolean(resource.error)&&<ErrorBox error={resource.error} retry={resource.reload}/>}
    {data&&data.mode==='source_preview'&&<>
      <div className="source-preview-summary"><span>Объектов: <strong>{data.coverage.propertyCount}</strong></span><span>Броней: <strong>{data.coverage.reservationCount}</strong></span><span>В периоде: <strong>{inPeriod.length}</strong></span><span>Данные на {new Date(data.asOf).toLocaleString('ru-RU')}</span>{nextBooking&&<Button onClick={()=>onJump(nextBooking.arrivalDate)}>К следующей брони ›</Button>}</div>
      <div className="calendar-scroll source-preview-scroll"><div className="calendar-grid" style={{minWidth:LABEL_WIDTH+DAY_COUNT*CELL_WIDTH}} role="grid" aria-label="Шахматка RealtyCalendar, только чтение">
        <div className="calendar-row calendar-head" role="row"><div className="calendar-label calendar-corner" role="columnheader"><strong>Объект RealtyCalendar</strong><small>{properties.length} из {data.coverage.propertyCount} · только чтение</small></div>{days.map(day=><div key={day} className={`calendar-head-cell ${[0,6].includes(new Date(`${day}T12:00:00Z`).getUTCDay())?'weekend':''}`} role="columnheader"><small>{new Intl.DateTimeFormat('ru-RU',{weekday:'short',timeZone:'UTC'}).format(new Date(`${day}T12:00:00Z`))}</small><strong>{Number(day.slice(8))}</strong></div>)}</div>
        {properties.map(property=>{
          const current=bookings.filter(item=>item.sourceLotId===property.sourceLotId&&intersects(item,start,end));
          return <div className="calendar-row source-preview-row" role="row" key={property.sourceLotId}><div className="calendar-label" role="rowheader"><strong title={property.label}>{property.label}</strong><small>{property.city||'Город не указан'} · объект RealtyCalendar</small></div>{days.map(day=><div key={day} className={`calendar-cell source-preview-cell ${[0,6].includes(new Date(`${day}T12:00:00Z`).getUTCDay())?'weekend':''}`} role="gridcell" aria-label={`${property.label}, ${dateRu(day)}; данные о свободных датах не подтверждены`}/>)}
