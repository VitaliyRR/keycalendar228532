...
import { SourcePreviewCalendar } from './SourcePreviewCalendar';
...
export function CalendarPage({ orgId, filtersOnly = false, canPreview = false }: { orgId: string; filtersOnly?: boolean; canPreview?: boolean }) {
  ...
  const showPreview=canPreview&&Boolean(payload&&payload.units.length===0);

  return <div className="calendar-page">
    <PageHeader title={filtersOnly?'Вид календаря и фильтры':'Шахматка'} description={showPreview?'Бронирования RealtyCalendar по датам объектов. Только чтение.':'Занятость по локальным датам объектов. Заявка не удерживает даты.'} actions={showPreview?<Link className="button button-primary" to="/import-preview">Все данные RealtyCalendar</Link>:<>{canPreview&&<Link className="button button-secondary" to="/import-preview">Данные RealtyCalendar</Link>}<Button variant="primary" onClick={()=>openCreate()}>+ Бронирование</Button></>} />
    ...
    <div className="calendar-filters"><Field label="Поиск объекта"><input type="search" value={search} onChange={event=>{setSearch(event.target.value); updateParams({search:event.target.value || null});}} placeholder="Название или адрес"/></Field>{!showPreview&&<Field label="Статус"><select value={status} onChange={event=>{setStatus(event.target.value);updateParams({status:event.target.value==='all'?null:event.target.value});}}><option value="all">Все записи</option><option value="confirmed">Подтверждённые</option><option value="request">Заявки</option><option value="cancelled">Отменённые</option></select></Field>}<span className="calendar-freshness">{showPreview?'Данные RealtyCalendar · только чтение':payload?.as_of ? `Обновлено ${new Date(payload.as_of).toLocaleString('ru-RU')}` : 'Данные загружаются по запросу'}</span></div>
    {resource.loading && <Loading/>}
    {Boolean(resource.error) && <ErrorBox error={resource.error} retry={resource.reload}/>}
    {!resource.loading && !resource.error && payload && units.length===0 && (showPreview?<SourcePreviewCalendar orgId={orgId} start={start} end={end} days={days} search={search} explicitStart={params.has('from')} onJump={day=>updateParams({from:day})}/>:<Empty title={payload.units.length ? 'По фильтрам ничего не найдено' : 'Пока нет объектов'} detail={payload.units.length ? 'Измените поиск или сбросьте фильтры.' : 'Добавьте объект и номер, чтобы увидеть календарь.'} action={<Button onClick={()=>navigate(screenPath(orgId,'SCR-OBJ-01'))}>Открыть объекты</Button>}/>)}
