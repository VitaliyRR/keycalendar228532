...
export function MigrationPage({orgId,canPreview=false}:{orgId:string;canPreview?:boolean}) {
  ...
  return <><PageHeader eyebrow="Перенос" title="Импорт и экспорт" description="Проверка исследует уже загруженный пакет и не переносит записи в рабочую базу." actions={canPreview?<Link className="button button-primary" to={screenPath(orgId,'SCR-MIG-02')}>Открыть предпросмотр</Link>:undefined}/>
